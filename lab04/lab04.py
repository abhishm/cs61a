...
def if_this_not_that(i_list, this):
    ...
    """
    "*** YOUR CODE HERE ***"
    print('\n'.join(str(i) if i > this else "that" for i in i_list))
...
def closer_city(lat, lon, city1, city2):
    ...
    return get_name(city2) if distance(new_city, city1) > distance(new_city, city2) else get_name(city1)
...
def print_board(board, max_rows, max_cols):
    ...
    for row in range(max_rows):
        ...
        for column in range(max_cols):
            columns.append(get_piece(board, row, column))
