...
def make_to_string(front, mid, back, empty_repr):
    ...
    def _repr(link):
        if link is Link.empty:
            ...
            return front + str(link.first) + mid + _repr(link.rest) + back
    return _repr
...
def add_trees(t1, t2):
    ...
    4
      ...
      5
    """
    common_len = min(len(t1.branches), len(t2.branches))
    return Tree(t1.root + t2.root, [add_trees(b1, b2) for b1, b2 in zip(t1.branches, t2.branches)] + t1.branches[common_len:]
                + t2.branches[common_len:]) 
