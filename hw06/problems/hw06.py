...
def balanced(m):
    ...
    if total_weight(end(l_side)) * length(l_side) != total_weight(end(r_side)) * length(r_side):
        return False
    elif is_weight(end(l_side)) and is_weight(end(r_side)):
        return True
    elif not is_weight(end(l_side)) and not is_weight(end(l_side)):
        return balanced(end(l_side)) and balanced(end(r_side))
    elif not is_weight(end(l_side)):
        return balanced(end(l_side))
    else:
        return balanced(end(r_side))
    ...
    def withdraw(amount, pwd):
        ...
        if len(incorrect_passwords) < 3:
            if pwd == password:
                ...
                incorrect_passwords.append(pwd)
                return "Incorrect password"
        else:
            return "Your account is locked. Attempts: {}".format(incorrect_passwords)
