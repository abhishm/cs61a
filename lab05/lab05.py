...
def num_songs(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    num_leaf = lambda t: 1 if is_leaf(t) else sum(num_leaf(b) for b in branches(t))
    return num_leaf(t)
...
def add_song(t, song, category):
    ...
    """
    "*** YOUR CODE HERE ***"
    branch = (root(t) == category) and branches(t) + [tree(song)] or branches(t)
    return tree(root(t), [add_song(b, song, category) for b in branch])
